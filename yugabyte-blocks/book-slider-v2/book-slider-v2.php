...
$block_data = yugabyte_block_data( $post_id, $block['name'], 'yugabyte-book-slider-v2-section yb-sec come-out' );
...
<style>
<?php
require 'book-slider-v2.css';
if ( 'ja' === YUGABYTE_CURRENT_LANGUAGE ) {
	require 'book-slider-v2-ja.css';
}
?>
</style>
</svg>

<div class="<?php echo esc_attr( $class_name ); ?>" data-lazy="book_slider_v2">
	<div class="container">
		<div class="yb-book-slider-v2-items owl-carousel">
			<?php
			foreach ( $acf_fields['book_slider_cards'] as $book_slider_card ) {
				if ( $book_slider_card ) {
					?>
				<div class="yb-book-slider-v2-item item">
          <div class="section-head">
            <h2><?php echo $book_slider_card['book_slider_heading']; ?></h2>
          </div>
						...
						<div class="yb-book-slide-details">
							...
							if ( ! empty( $book_slider_card['slider_points'] ) ) {
								...
								foreach ( $book_slider_card['slider_points'] as $slider_point ) {
									if ( $slider_point ) {
										echo '<div class="yb-book-slide-point">' .
											$slider_point['point'] . '
										</div>';
									}
								}
...
<script>
function book_slider_v2() {
	yugabyteLoadJs('/wp-content/themes/yugabyte/blocks/book-slider-v2/book-slider-v2.js?<?php echo $theme_version; ?>', 'BODY','book_slider_v2');
}
</script>
